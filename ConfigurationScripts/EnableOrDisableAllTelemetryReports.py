#
# EnableOrDisableAllTelemetryReports.py Python script using Redfish API to Enable or Disable All Telemetry Reports with Default/Existing settings.
#
#
#
...
parser = argparse.ArgumentParser( description = "Python script using Redfish to Enable/Disable all iDRAC Telemetry reports. No other configuration changes are made.")
...
parser.add_argument('-s',
                    help = 'Pass in the report status to be set. Possible values are Enabled/Disabled', default = 'Enabled',required = False)
...
def get_attributes():
    ...
    try:
        ...
        telemetry_attributes = {k: v for k, v in attributes.items() if ((k.startswith('Telemetry')) and (k.endswith("EnableTelemetry")))}
        logging.info("iDRAC Telemetry is currently '{}'.".format(telemetry_attributes.get('Telemetry.1.EnableTelemetry','Unknown')))
...
def set_attributes(telemetry_attributes):
    ...
    if not status_to_set in ['Enabled','Disabled']:
    ...
    response = requests.patch(url, data=json.dumps({"Attributes":updated_telemetry_attributes}), headers = headers, verify = False,auth = (idrac_username, idrac_password))
    if response.status_code != 200:
        logging.error("FAIL, status code for reading attributes is not 200, code is: {}".format(response.status_code))
...
if __name__ == "__main__":
    telemetry_attributes = get_attributes()
    set_attributes(telemetry_attributes)