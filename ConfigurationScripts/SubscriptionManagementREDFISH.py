    ...
    while True:
        ...
        if "failed" in final_message_string or "completed with errors" in final_message_string or "Not one" in final_message_string:
            ...
            try:
                ...
                print("- Message = %s" % message_string[len(message_string)-1]["Message"])
                sys.exit()
            break
        else:
...
def get_set_ipmi_alert_iDRAC_setting():
    ...
    while True:
        ...
        print("- INFO, checking current value for iDRAC attribute \"IPMILan.1.AlertEnable\"")

        if attributes_dict["IPMILan.1.AlertEnable"] == "Disabled":
            print("- INFO, current value for iDRAC attribute \"IPMILan.1.AlertEnable\" is set to Disabled, setting value to Enabled")
            ...
            print("- INFO, current value for iDRAC attribute \"IPMILan.1.AlertEnable\" already set to Enabled, ignore PATCH command")
            break
...
def submit_test_event():
    ...
    headers = {'content-type': 'application/json'}
    response = requests.post(url, data=json.dumps(payload), headers=headers, verify=False,auth=(idrac_username, idrac_password))
    if response.__dict__["status_code"] == 200 or response.__dict__["status_code"] == 202 or response.__dict__["status_code"] == 204:
        print("\n- PASS, POST command passed, status code %s returned, event type \"%s\" successfully sent to destination \"%s\"" % (response.__dict__["status_code"],event_type, destination))
    else:
