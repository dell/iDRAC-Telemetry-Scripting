...
parser = argparse.ArgumentParser(description="Python script using Redfish API to either get event service properties,"
                                             " get event subscriptions, create / delete subscriptions or submit test "
                                             "event.")
parser.add_argument('-ip', help='iDRAC IP address', required=True)
parser.add_argument('-u', help='iDRAC username', required=True)
parser.add_argument('-p', help='iDRAC password', required=True)
parser.add_argument('script_examples', action="store_true", help='SubscriptionManagementREDFISH.py -ip 192.168.0.120 '
                    '-u root -p calvin -s yy, this example will get current subscription URIs and details. '
                    'SubscriptionManagementREDFISH.py -ip 192.168.0.120 -u root -p calvin -c y -D '
                    'https://192.168.0.130 -E Alert -V MetricReport, this example will create a MetricReport '
                    'subscription for alert events which will use 192.168.0.130 Redfish event listener. '
                    'SubscriptionManagementREDFISH.py -ip 192.168.0.120 -u root -p calvin --delete '
                    '/redfish/v1/EventService/Subscriptions/c1a71140-ba1d-11e9-842f-d094662a05e6, this example will '
                    'delete a subscription')
parser.add_argument('-e', help='Get event service properties, pass in \"y\"', required=False)
parser.add_argument('-s',
                    help='Get event service subscriptions URIs, pass in \"y\". To get detailed information for '
                         'each subscription URI, pass in \"yy\"',
                    required=False)
parser.add_argument('-c',
                    help='Create subscription, pass in \"y\". You must also use agruments -D, -V and -E to create a '
                         'subscription',
                    required=False)
parser.add_argument('-t',
                    help='Submit test event, pass in \"y\". You must also use arguments -D, -E and -M to submit a test'
                         ' event',
                    required=False)
parser.add_argument('-D', help='Pass in Destination HTTPS URI path for either create subscription or send test event',
                    required=False)
parser.add_argument('-V',
                    help='Pass in Event Format Type for creating a subscription. Supported values are \"Event\", '
                         '\"MetricReport\" or \"None\"',
                    required=False)
parser.add_argument('-E',
                    help='Pass in EventType value for either create subscription or send test event. Supported values'
                         ' are StatusChange, ResourceUpdated, ResourceAdded, ResourceRemoved, Alert, MetricReport.',
                    required=False)
parser.add_argument('-M', help='Pass in MessageID for sending test event. Example: TMP0118', required=False)
parser.add_argument('--delete',
                    help='Pass in complete service subscription URI to delete. Execute -s argument if needed to get '
                         'subscription URIs',
                    required=False)
...
def get_event_service_subscriptions():
    ...
    for i in data["Members"]:
        for ii in i.items():
            if args["s"] == "yy":
                response = requests.get('https://%s%s' % (idrac_ip, ii[1]), verify=False,
                                        auth=(idrac_username, idrac_password))
                data = response.json()
...
def delete_subscriptions():
    ...
    if response.__dict__["status_code"] == 200:
        ...
        print("\n- FAIL, DELETE command failed and returned status code %s, error is %s" % (
...
def scp_set_idrac_attribute():
    ...
    while True:
        ...
        if status_code == 202 or status_code == 200:
            ...
            print("Query job ID command failed, error code is: %s" % status_code)
            sys.exit()
...
def get_set_ipmi_alert_iDRAC_setting():
    ...
    while True:
        try:
            ...
            print("\n- WARNING, iDRAC version detected does not support PATCH to set iDRAC attributes, executing "
                  "Server Configuration Profile feature set iDRAC attribute \"IPMILan.1#AlertEnable\" locally\n")
        ...
        if attributes_dict["IPMILan.1.AlertEnable"] == "Disabled":
            print("- WARNING, current value for iDRAC attribute \"IPMILan.1.AlertEnable\" is set to Disabled, setting"
                  " value to Enabled")
            ...
            print("- WARNING, current value for iDRAC attribute \"IPMILan.1.AlertEnable\" already set to Enabled, "
...
def create_subscription():
    ...
    if response.__dict__["status_code"] == 201:
        ...
        print("- FAIL, POST command failed, status code %s returned, error is %s" % (
        response.__dict__["status_code"], response.__dict__["_content"]))
        sys.exit()
...
def submit_test_event():
    ...
    response = requests.post(url, data=json.dumps(payload), headers=headers, verify=False,
                             auth=(idrac_username, idrac_password))
    if response.__dict__["status_code"] == 201:
        print("\n- PASS, POST command passed, status code 201 returned, event type \"%s\" successfully sent to "
              "destination \"%s\"" % (event_type, destination))
    else:
        print("\n- FAIL, POST command failed, status code %s returned, error is %s" % (
        response.__dict__["status_code"], response.__dict__["_content"]))
        sys.exit()
